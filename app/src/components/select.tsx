        ...
        <div className="col-sm-12">
          ...
          >
            ...
            {props.options.map((val: string) => (
              <option selected={value == val} value={val}>
                {val}
              </option>
            ))}
          </select>
          ...
          <input
            type="hidden"
            name="daily_register_id"
            id="daily_register_id"
          />
          {showError && (
            <div
              style={{ marginLeft: 10, fontSize: 13 }}
              id={`${id}-error`}
              className="demo-form-feedback text-danger"
            >
              {errorMessage}
            </div>
          )}
